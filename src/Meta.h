...
namespace Adversity
{
		...
		inline YAML::Node Write() const
		{
			...
			for (const auto& [key, value] : _data) {
				...
				switch (index) {
					...
					case DataType::Form:
					{
						node[key] = Util::GetFormIDString(std::get<RE::TESForm*>(value));
						break;
					}
					case DataType::BoolList:
					{
					...
					case DataType::FormList:
					{
						...
						for (const auto& form : forms) {
							values.push_back(form ? Util::GetFormIDString(form) : "none");
						}
	...
	private:
		static GenericData ConvertToGeneric(const std::string& a_str)
		{
			...
			if (Util::Lower(a_str) == "none") {
				value = nullptr;
			} else if (const auto form = Util::GetFormFromString(a_str)) {
				value = form;
